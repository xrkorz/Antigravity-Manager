...
/// Antigravity 4.1.33 uses Electron 39.2.3 which corresponds to Chrome 132.0.6834.160
...
/// Current resolved Antigravity version (e.g., "4.1.33")
...
/// "Antigravity/4.1.33 (Macintosh; Intel Mac OS X 10_15_7) Chrome/132.0.6834.160 Electron/39.2.3"
...
/// Version selection: max(local installation, remote latest, known stable 4.1.33)
...
mod tests {
    ...
    fn test_compare_semver() {
        assert_eq!(compare_semver("4.1.33", "4.1.22"), std::cmp::Ordering::Greater);
        assert_eq!(compare_semver("4.1.22", "4.1.33"), std::cmp::Ordering::Less);
        assert_eq!(compare_semver("4.1.33", "4.1.33"), std::cmp::Ordering::Equal);
    ...
    fn test_old_local_version_uses_floor() {
        // Simulate: local = 4.1.20 (old), floor = 4.1.33
        // Expected: use floor
        let local = "4.1.20";
    ...
    fn test_newer_local_version_takes_priority() {
        // Simulate: local = 4.1.33 (newer than floor), floor = 4.1.33
        // Expected: use local
        let local = "4.1.33";
        let floor = KNOWN_STABLE_VERSION;
        let best = if compare_semver(local, floor) >= std::cmp::Ordering::Equal {
